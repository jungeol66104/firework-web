...
export function UsersDataTable() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const result = await fetchAllUsers(supabase, { limit: 50 })
  ...
  const handleInterviewsClick = (userId: string, userName: string) => {
  ...
  const columns = createUsersColumns(handleInterviewsClick, handleUserClick)
